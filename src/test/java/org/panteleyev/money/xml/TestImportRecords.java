...
public class TestImportRecords extends BaseDaoTest {
    ...
    private void compareDatabase(String expected) throws Exception {
        ...
        try (var inputStream = new FileInputStream(file)) {
            ...
            Assert.assertEquals(BaseTest.sortedById(cache().getCategories()), BaseTest.sortedById(imp.getCategories()));
            Assert.assertEquals(BaseTest.sortedById(cache().getAccounts()), BaseTest.sortedById(imp.getAccounts()));
            Assert.assertEquals(BaseTest.sortedById(cache().getCurrencies()), BaseTest.sortedById(imp.getCurrencies()));
            Assert.assertEquals(BaseTest.sortedById(cache().getContacts()), BaseTest.sortedById(imp.getContacts()));
            Assert.assertEquals(BaseTest.sortedById(cache().getTransactions()), BaseTest.sortedById(imp.getTransactions()));
        }
    }
}
